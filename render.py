...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline,
                           background, cam_type=cam_type)["render"]
    ...
    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(
        iteration), 'video_rgb.mp4'), render_images, fps=30)
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter,
                       scene.getTrainCameras(), gaussians, pipeline, background, cam_type)
        ...
        if not skip_video:
            render_set(dataset.model_path, "video", scene.loaded_iter,
                       scene.getVideoCameras(), gaussians, pipeline, background, cam_type)
