...
def init_3DGaussians_ply(points, scales, rotations, opactiy, shs, feature_shape):
    ...
    f_dc = shs[:, :feature_shape[0], :].detach().transpose(
        1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    # breakpoint()
    f_rest = shs[:, feature_shape[0]:, :].detach().transpose(
        1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
...
gaussians, scene = render_sets(model.extract(args), hyperparam.extract(
    args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_video)
...
for index, viewpoint in enumerate(scene.getTestCameras()):
    ...
    gs_ply = init_3DGaussians_ply(points, scales_final, rotations_final, opacity_final, shs_final, [
