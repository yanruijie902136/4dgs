...
class Load_hyper_data(Dataset):
    def __init__(self,
        ...
        self.all_time = [meta_json[i]['warp_id'] /
                         max_time for i in self.all_img]
        ...
        self.all_depth = [
            f'{datadir}/depth/{int(1/ratio)}x/{i}.npy' for i in self.all_img]

        self.all_img = [
            f'{datadir}/rgb/{int(1/ratio)}x/{i}.png' for i in self.all_img]

        self.h, self.w = self.all_cam_params[0].image_shape
        self.map = {}
        self.image_one = Image.open(self.all_img[0])
        self.image_one_torch = PILtoTorch(
            self.image_one, None).to(torch.float32)
        if os.path.exists(os.path.join(datadir, "covisible")):
            self.image_mask = [
                f'{datadir}/covisible/{int(2)}x/val/{i}.png' for i in self.all_img_origin]
        else:
    ...
    def generate_video_path(self):
        ...
        self.select_video_cams = [item for i, item in enumerate(
            self.all_cam_params) if i % 1 == 0]
        self.video_path, self.video_time = smooth_camera_poses(
            self.select_video_cams, 10)
        # breakpoint()
    ...
    def load_raw(self, idx):
        ...
        if self.image_mask is not None and self.split == "test":
            ...
            mask = F.interpolate(mask.unsqueeze(0), size=[
                                 self.h, self.w], mode='bilinear', align_corners=False).squeeze(0)
        else:
            mask = None
...
def format_hyper_data(data_class, split):
    ...
    for uid, index in tqdm(enumerate(data_idx)):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=None,
                              image_path=image_path, image_name=image_name, width=int(
                                  data_class.w),
