...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        self.image_paths, self.image_poses, self.image_times, N_cam, N_time = self.load_images_path(
            videos, self.split)
    ...
    def get_val_pose(self):
        ...
        render_times = torch.linspace(
            0.0, 1.0, render_poses.shape[0]) * 2.0 - 1.0
    ...
    def load_images_path(self, videos, split):
        ...
        for index, video_path in enumerate(videos):
            ...
            if not os.path.exists(image_path):
                print(
                    f"no images saved in {image_path}, extract images from video.")
                os.makedirs(image_path)
                this_count = 0
                while video_frames.isOpened():
                    ret, video_frame = video_frames.read()
                    ...
                    if ret:
                        ...
                        if self.downsample != 1.0:
                            ...
                            img = video_frame.resize(
                                self.img_wh, Image.LANCZOS)
                        img.save(os.path.join(image_path, "%04d.png" % count))
