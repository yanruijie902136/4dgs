...
def voxel_down_sample_custom(points, voxel_size):
    ...
    unique_voxels, inverse_indices = torch.unique(
        voxel_grid, dim=0, return_inverse=True)
...
def downsample_point_cloud(points, ratio):
    ...
    downsampled_dataset, _ = random_split(
        dataset, [num_downsampled_points, num_points - num_downsampled_points])
...
def downsample_point_cloud_open3d(points, voxel_size):
    downsampled_pcd, inverse_indices = voxel_down_sample_custom(
        points, voxel_size)
...
def upsample_point_cloud(points, density_threshold, displacement_scale, iter_pass):
    try:
        nbrs = NearestNeighbors(n_neighbors=2+iter_pass,
...
def visualize_point_cloud(points, low_density_points, new_points):
    ...
    all_points = np.concatenate(
        [points, low_density_points, new_points], axis=0)
    pcd.points = o3d.utility.Vector3dVector(all_points)
    colors = np.zeros((all_points.shape[0], 3))
    colors[:points.shape[0]] = [0, 0, 0]
    colors[points.shape[0]:points.shape[0] +
           low_density_points.shape[0]] = [1, 0, 0]
...
def addpoint(point_cloud, density_threshold, displacement_scale, iter_pass,):
    points, low_density_points, new_points, low_density_index = upsample_point_cloud(
        point_cloud, density_threshold, displacement_scale, iter_pass)
...
def find_point_indices(origin_point, goal_point):
    indices = torch.nonzero(
        (origin_point[:, None] == goal_point).all(-1), as_tuple=True)[0]
    return indices
...
if __name__ == "__main__":
    ...
    for i in range(pass_+1, 50):
        ...
        while point_downsample.shape[0] > 1000:
            ...
            point_downsample = downsample_point_cloud_open3d(
                point_cloud, voxel_size=voxel_size)
            flag = True

        print("point size:", point_downsample.shape[0])
        downsampled_point_index = find_indices_in_A(
            point_cloud, point_downsample)
        print("selected_num", point_cloud[downsampled_point_index].shape[0])
        _, low_density_points, new_points, low_density_index = addpoint(
            point_cloud[downsampled_point_index], density_threshold=density_threshold, displacement_scale=displacement_scale, iter_pass=0)
